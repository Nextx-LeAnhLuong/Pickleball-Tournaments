...
| ReplacedByTokenId | INTEGER | ✅ | NULL | Token thay thế (rotation) |
...
|-----|--------------|------|---------|--------|
| **Id** | INTEGER | ❌ | Auto-increment | Khóa chính |
| FollowerId | INTEGER | ❌ | | Người theo dõi → Users |
| FollowingId | INTEGER | ❌ | | Người được theo dõi → Users |
...
|-----|--------------|------|---------|--------|
| **Id** | INTEGER | ❌ | Auto-increment | Khóa chính |
| CreatorId | INTEGER | ❌ | | FK → Users (người tạo giải) |
...
| BannerUrl | VARCHAR(500) | ✅ | NULL | Ảnh bìa Cloudinary |
...
| InvitedBy | INTEGER | ✅ | NULL | FK → Users (người mời, NULL nếu tự xin) |
...
| PlayerId | INTEGER | ✅ | NULL | FK → Users (Singles), NULL nếu Doubles |
| TeamId | INTEGER | ✅ | NULL | FK → Teams (Doubles), NULL nếu Singles |
...
| Player1Id | INTEGER | ❌ | | User ID (Singles) hoặc Team ID (Doubles) |
| Player2Id | INTEGER | ❌ | | User ID (Singles) hoặc Team ID (Doubles) |
| Player1Scores | JSONB | ✅ | NULL | Điểm từng set, vd: `[11, 9, 11]` |
| Player2Scores | JSONB | ✅ | NULL | Điểm từng set, vd: `[7, 11, 8]` |
| WinnerId | INTEGER | ✅ | NULL | ID người/đội thắng |
...
| ModifiedBy | INTEGER | ❌ | | FK → Users (người sửa) |
...
|-----|--------------|------|---------|--------|
| **Id** | INTEGER | ❌ | Auto-increment | Khóa chính |
| GameId | INTEGER | ❌ | | FK → CommunityGames |
| UserId | INTEGER | ❌ | | FK → Users |
...
|-----|--------------|------|---------|--------|
| **Id** | INTEGER | ❌ | Auto-increment | Khóa chính |
| RoomId | INTEGER | ❌ | | FK → ChatRooms |
| UserId | INTEGER | ❌ | | FK → Users |
| JoinedAt | TIMESTAMPTZ | ❌ | `NOW()` | |
| MutedUntil | TIMESTAMPTZ | ✅ | NULL | Tắt thông báo đến thời điểm |
| LastReadMessageId | INTEGER | ✅ | NULL | Tin nhắn cuối đã đọc |
...
|-----|--------------|------|---------|--------|
| **Id** | INTEGER | ❌ | Auto-increment | Khóa chính |
| UserId | INTEGER | ❌ | | FK → Users (người nhận) |
...
('player1@demo.com', '$2a$12$...', 'Nguyễn Văn A', 3.5),
('player2@demo.com', '$2a$12$...', 'Trần Văn B', 4.0),
('player3@demo.com', '$2a$12$...', 'Lê Văn C', 3.0),
('player4@demo.com', '$2a$12$...', 'Phạm Văn D', 3.5);

-- 1 demo tournament (singles, 1 bảng, completed)
-- Giả sử user 1 (Nguyễn Văn A) là người tạo giải
